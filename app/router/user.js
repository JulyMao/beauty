...
module.exports = app => {
  ...
  // 用户相关
  apiRouter.post('/user/login', passportLocal, controller.user.login); // 登录
  apiRouter.post('/user/logout', loginRequired, controller.user.logout); // 登出
  apiRouter.post('/user/add', middleware.adminUser, loginRequired, controller.user.addUser); // 添加用户
  apiRouter.post('/user/del', middleware.adminUser, loginRequired, controller.user.delUser); // 删除用户
  ...
  apiRouter.get('/user/list', middleware.adminUser, loginRequired, middleware.pagination, controller.user.userList); // 获取用户列表
